...
export default function ButterToastStarterProposalPage() {
  ...
  useEffect(() => {
    sendProposalAuditTrack("butter-toast-starter", "PAGE_VISITED_SESSION").then((res) => {
  ...
  const VALID_PASSWORDS = ["20260723", "0723", "20260724", "0724", "20260725", "0725"];
  ...
  useEffect(() => {
    ...
    if (unlocked === "true") {
      setIsUnlocked(true);
      sendProposalAuditTrack("butter-toast-starter", "PAGE_VISITED_SESSION");
    }
      ...
      <div className="w-full min-h-screen bg-[#F7F3ED] text-[#382D24] flex flex-col justify-center items-center p-4 font-sans select-none overflow-x-hidden">
        ...
        <div className="w-full max-w-sm bg-[#FFFDF9] border border-[#E6DDCF] rounded-3xl p-6 shadow-xl text-center backdrop-blur-md">
          {lifecycleState.stage === "EXPIRED" ? (
            <>
              <div className="w-12 h-12 bg-amber-100 text-amber-800 rounded-full flex items-center justify-center text-xl mx-auto mb-3 border border-amber-300">
                🔒
              </div>
              <h1 className="text-lg font-bold font-serif mb-1.5 text-rose-700">
                🔒 專案報價存取期限已過期失效
              </h1>
              <p className="text-xs text-[#7C6E62] mb-4 leading-relaxed font-medium">
              ...
              <div className="space-y-3">
                <input
                  ...
                  disabled
                  placeholder="密碼已自動停用"
                  className="w-full px-3 py-2.5 bg-[#F7F3ED] border border-[#E6DDCF] rounded-xl text-center text-base opacity-50 cursor-not-allowed text-[#A39587]"
                />
                <button
                  disabled
  ...
  return (
    <ProposalEditableProvider slug="butter-toast-starter" isAdminBypass={isAdminBypass}>
      <div className="w-full min-h-screen bg-[#F7F3ED] print:bg-white text-[#382D24] font-sans select-none overflow-x-hidden">
          ...
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xs md:text-sm text-[#382D24]">
                <EditableText id="bts_header_title" defaultText="AI 品牌店長 基礎入門版專案 (奶油吐司精簡版)" />
              </span>
            </div>
        ...
        <div className="block md:hidden w-full max-w-xl mx-auto p-3 space-y-4 print:hidden">
          ...
          {sectionChecklist}
          {sectionSummary}
          <PrintSignatureSection proposalTitle="【奶油吐司】AI 品牌店長 基礎精簡版專案" />
        </div>
