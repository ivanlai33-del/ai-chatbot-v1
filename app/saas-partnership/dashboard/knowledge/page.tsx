...
import { ORG_ID } from '@/context/PartnerContext';
...
const PRO_BADGE = (
    <span className="ml-2 px-2 py-0.5 rounded-full text-[9px] font-black bg-emerald-100 text-emerald-600 border border-emerald-200 uppercase tracking-widest">PRO</span>
);
...
export default function KnowledgeHub() {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                // Fetch the specific bot for this organization
                ...
                if (bots) {
                    setBotId(bots.id);
                    setMgmtToken(bots.mgmt_token);
                    setSelectedPlan(bots.selected_plan || '');
                    setBasePrompt(bots.system_prompt || '');

                    // Fetch FAQs linked to this specific bot
                    ...
                    // Fetch Reservations linked to this specific bot
        ...
        return (
            <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5
                    ${activeTab === id ? 'bg-[#06C755] text-white shadow-lg shadow-[#06C755]/20' : 'text-slate-500 hover:text-slate-900'}
    ...
    return (
        <div className="p-8 flex flex-col lg:flex-row gap-8 min-h-full">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                    <div>
                        <motion.h1
                            ...
                            className="text-3xl font-black text-slate-900 tracking-tight mb-2 flex items-center gap-3"
                        >
                            <BrainCircuit className="w-8 h-8 text-[#06C755]" />
                            AI 店長智庫
                        </motion.h1>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative group">
                            ...
                            <input type="text" placeholder="搜尋知識點..." className="bg-white border border-slate-200 rounded-2xl py-2.5 pl-10 pr-4 text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#06C755]/10 transition-all min-w-[180px]" />
                        </div>
                    </div>
                </header>

                <div className="flex flex-wrap items-center gap-2 mb-10 bg-white/60 backdrop-blur-xl p-1.5 rounded-2xl w-fit border border-slate-200/60 shadow-sm">
                            ...
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                {KNOWLEDGE_CONFIG.templates.map((tpl, i) => (
                                    <motion.div
                                        ...
                                        className="group p-8 bg-white/60 rounded-[2.5rem] border border-slate-200/60 backdrop-blur-xl relative overflow-hidden cursor-pointer hover:border-[#06C755]/30 transition-all shadow-sm hover:shadow-[#06C755]/5"
                                    >
                                        <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${tpl.color} opacity-5 blur-3xl group-hover:opacity-10 transition-opacity`} />
                                        <div className="text-4xl mb-6">{tpl.icon}</div>
                                        <h3 className="text-xl font-black text-slate-900 mb-3 tracking-tight group-hover:text-[#06C755] transition-colors">{tpl.title}</h3>
                                        <p className="text-sm text-slate-500 leading-relaxed font-medium mb-8">{tpl.desc}</p>
                                        <button className="flex items-center gap-2 text-[10px] font-black text-[#06C755] uppercase tracking-widest group-hover:gap-3 transition-all">
            ...
            <aside className="hidden xl:flex w-80 lg:w-96 bg-white/60 backdrop-blur-3xl border border-slate-100 rounded-[3rem] flex-col h-fit shrink-0 overflow-hidden shadow-2xl">
