...
export const PricingWidget: React.FC<PricingWidgetProps> = ({ billingCycle, onToggleBilling, onSelectPlan, lineUserId, initiateLineLogin }) => {
    ...
    const plans = SHOW_PLAN_IDS.map(id => {
        ...
        return {
            name: p.name,
            price: isYearly ? p.pricing.annual.toLocaleString() : p.pricing.monthly.toLocaleString(),
        ...
        <div className="ml-14 max-w-[85%] space-y-6">
            ...
            <div className="flex p-1 bg-zinc-100/80 backdrop-blur-sm rounded-2xl w-fit mx-auto shadow-inner border border-white">
                <button
                    ...
                    className={cn(
                        "px-8 py-2.5 rounded-[14px] text-base font-black transition-all",
...
export const CheckoutWidget: React.FC<{ selectedPlan: any, billingCycle: string, onPayment: () => void, lineGreen: string }> = ({ selectedPlan, billingCycle, onPayment, lineGreen }) => (
    ...
    >
         <div className="flex items-center gap-3 font-black text-2xl" style={{ color: lineGreen }}>
        ...
        <div className="space-y-4">
            <div className="bg-zinc-50 p-4 rounded-xl border-2 border-[#06C755] flex justify-between items-center mb-2 shadow-sm shadow-green-500/5">
                 <span className="text-zinc-500 font-bold text-lg">已選方案</span>
                <div className="text-right">
                     <div className="font-black text-zinc-900 text-2xl">{selectedPlan.name || '標準型'}</div>
                </div>
            </div>
            {/* Input fields... truncated for brevity but should include card fields */}
             <input type="text" placeholder="XXXX XXXX XXXX XXXX" className="w-full p-4 rounded-xl border-2 border-zinc-200 bg-zinc-50 text-xl outline-none" />
        </div>
        <button
            onClick={onPayment}
             className="w-full py-5 text-white rounded-2xl font-black text-[24px] shadow-lg"
...
export const SuccessWidget: React.FC<{ botId: string, mgmtToken: string, isAdminView: boolean, onAdminLogin: () => void }> = ({ botId, mgmtToken, isAdminView, onAdminLogin }) => (
    ...
    >
         <div className="flex items-center gap-3 font-black text-2xl text-[#06C755]">
            <Sparkles className="w-7 h-7" />
            <span>恭喜！您的 AI 店長已待命</span>
        </div>
        <div className="bg-[#06C755] p-6 rounded-2xl border border-[#06C755] space-y-3 shadow-lg">
             <p className="text-[15px] font-black text-white uppercase tracking-widest text-center">您的專屬 Webhook 網址</p>
             <div className="bg-white p-4 rounded-xl text-center font-mono text-[16px] break-all">
        ...
        {!isAdminView && (
            <button
                onClick={onAdminLogin}
                 className="w-full py-4 text-white rounded-xl font-black text-lg shadow-lg"
        ...
        <div className="relative bg-white/90 backdrop-blur-md p-8 rounded-[32px] border border-emerald-100 shadow-2xl space-y-6">
            <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-200">
                 ...
                 <div>
                    <h3 className="text-2xl font-black text-slate-800 leading-tight">AI 店長靈魂已建構完成</h3>
                    <p className="text-[15px] text-slate-500 font-bold mt-1">品牌 DNA、行業知識、接待計畫已就緒</p>
                </div>
            </div>
             ...
             <button
                onClick={onEnterHub}
                className="w-full py-5 bg-gradient-to-r from-[#06C755] to-emerald-600 hover:from-emerald-600 hover:to-teal-600 text-white rounded-2xl font-black text-[22px] shadow-xl shadow-emerald-100 transition-all active:scale-95 flex items-center justify-center gap-3"
            >
                進入店長智庫預覽成果 ➔
            </button>
             <p className="text-xs text-center text-slate-400 font-bold tracking-widest uppercase">無需付費 · 立即預覽 AI 人設</p>
        </div>
    </motion.div>
);
