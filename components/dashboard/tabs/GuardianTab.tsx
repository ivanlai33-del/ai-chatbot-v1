...
export default function GuardianTab({ botId }: { botId: string }) {
    ...
    const handleScan = async () => {
        ...
        try {
            ...
            console.error('Scan guardian error:', error);
        } finally {
            setIsScanning(false);
        }
    };
    ...
    const sentimentData = [
        { label: '正面情緒', percent: Math.round(metrics.positive_ratio * 100), color: 'bg-emerald-500' },
        { label: '中立情緒', percent: Math.round(metrics.neutral_ratio * 100), color: 'bg-slate-300' },
        { label: '負面情緒', percent: Math.round(metrics.negative_ratio * 100), color: 'bg-rose-500' },
    ];

    const recentMentions = data?.mentions || [];
    const monitoringKeywords = data?.keywords || [];
                ...
                <div className="lg:col-span-2 p-8 rounded-[40px] bg-white border border-slate-100 shadow-sm relative overflow-hidden">
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-[22px] font-black text-slate-800">品牌護衛監控中</h3>
                                <p className="text-[12px] text-slate-400 font-bold tracking-widest uppercase">7/24 Active Surveillance</p>
                            </div>
                        </div>
                        <button 
                            onClick={handleScan}
                            disabled={isScanning}
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group disabled:opacity-50"
                        >
                             <div className={`h-2 w-2 rounded-full bg-emerald-500 ${isScanning ? 'animate-ping' : 'animate-pulse'}`} />
                             <span className="text-[14px] font-black text-emerald-600 uppercase group-hover:scale-105 transition-transform">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            <div className="text-[48px] font-black text-slate-900 leading-none mb-2">{metrics.health_score}<span className="text-[20px] text-slate-400 ml-1">/100</span></div>
                            <div className="text-[13px] font-black text-slate-500 uppercase tracking-wider mb-4">品牌健康指數</div>
                        ...
                        <div className="md:col-span-2 flex flex-col justify-end gap-3">
                            {sentimentData.map((s, i) => (
                                <div key={i} className="flex items-center gap-4">
                                    <span className="w-20 text-[12px] font-black text-slate-500">{s.label}</span>
                                    ...
                                    <span className="w-10 text-[12px] font-black text-slate-800 text-right">{s.percent}%</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button 
                                ...
                                className={`w-12 h-6 rounded-full transition-colors relative ${notificationsEnabled ? 'bg-emerald-500' : 'bg-white/20'}`}
                            >
                                <motion.div 
                        ...
                        <h4 className="text-[20px] font-black mb-2">危機警報推送</h4>
                        <p className="text-white/60 text-[14px] font-medium leading-relaxed">
                            當系統偵測到高風險負評或社群提及時，將立即透過 LINE 告知您。
                        </p>
                    </div>
                    
                    <button className="w-full py-4 mt-6 rounded-[24px] bg-white/10 hover:bg-white/20 border border-white/20 transition-all font-black text-[14px] flex items-center justify-center gap-2">
                ...
                <div className="p-8 rounded-[32px] bg-white border border-slate-100 max-h-[400px] overflow-hidden flex flex-col">
                    ...
                    <div className="space-y-4 overflow-y-auto pr-2 glass-scrollbar">
                        {recentMentions.length > 0 ? recentMentions.map((alert: any, i: number) => (
                            ...
                            <p className="text-slate-400 text-center font-bold">目前無提及錄</p>
                        )}
                    </div>
                </div>
            </div>
