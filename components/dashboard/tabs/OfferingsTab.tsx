        ...
        <div className="space-y-6">
            ...
            <div className="p-6 rounded-[32px] bg-gradient-to-br from-slate-900 to-slate-800 text-white shadow-xl shadow-slate-200/50">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-[12px] text-slate-400 font-bold uppercase tracking-wider text-left">Auto-Sync via Crawler</p>
                    </div>
                </div>
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between px-2 mb-1">
                    <p className="text-[12px] font-black text-slate-400 uppercase tracking-widest text-left">商品 / 服務清單</p>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="flex-1 flex flex-col min-w-0">
                                    <span className={`text-[18px] font-black tracking-tight truncate text-left ${hasName ? 'text-slate-900' : 'text-slate-400'}`}>
                                        {item.name || '新增商品 / 服務項目'}
                                    </span>
                                    {item.category && (
                                        <span className="text-[12px] text-slate-400 font-bold uppercase tracking-widest mt-1 text-left">{item.category}</span>
                                    )}
                                </div>
                                {item.price && !open && (
                                    <div className="hidden sm:flex flex-col items-end mr-6">
                                        <span className="text-[11px] text-slate-400 font-black uppercase tracking-widest mb-1 text-right">PRICE</span>
                                        <span className="text-[17px] text-slate-900 font-black text-right">NT$ {item.price}</span>
                                    </div>
                                )}
                                <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                                    <button onClick={() => duplicate(i)} className="p-3 rounded-[24px] bg-white border border-slate-100 text-slate-400 hover:text-indigo-600 hover:border-indigo-100 transition-all shadow-sm"><Copy className="w-4 h-4" /></button>
                                    <button onClick={() => remove(i)} className="p-3 rounded-[24px] bg-white border border-slate-100 text-slate-400 hover:text-red-500 hover:border-red-100 transition-all shadow-sm"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            </div>
                                ...
                                {open && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                        <div className="px-5 pb-5 pt-2 space-y-4 border-t border-slate-100">
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 tracking-widest uppercase mb-2 text-left">基本資訊</p>
                                                <div className="grid grid-cols-2 gap-3">
                                                    {BASIC_FIELDS.map(f => <InputField key={f.key} label={f.label} placeholder={f.placeholder} value={item[f.key] || ''} onChange={v => update(i, f.key, v)} />)}
                                                </div>
                                            </div>
                                            <TextareaField label="商品說明（對外顯示）" placeholder="詳細介紹您的產品或服務特色..." value={item.description} onChange={v => update(i, 'description', v)} />
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 tracking-widest uppercase mb-2 text-left">進階設定（選填）</p>
                                                <div className="grid grid-cols-2 gap-3">
                                                    {DETAIL_FIELDS.map(f => <InputField key={f.key} label={f.label} placeholder={f.placeholder} value={item[f.key] || ''} onChange={v => update(i, f.key, v)} />)}
                                                </div>
                                            </div>
                                            <div className="p-3 rounded-[24px] bg-slate-50 border border-dashed border-slate-200">
                                                <TextareaField label="AI 備註" placeholder="..." value={item.ai_context} onChange={v => update(i, 'ai_context', v)} rows={2} />
                                            </div>
                                        </div>
                                    </motion.div>
                                )}
            ...
            {atLimit ? (
                <div className="w-full flex flex-col items-center justify-center gap-3 py-8 rounded-[24px] border-2 border-dashed border-amber-200 bg-amber-50">
                    <div className="flex items-center gap-2 text-amber-600">
                        ...
                        <span className="text-[15px] font-black">已達 {formatLimit(productLimit)} 項上限</span>
                    </div>
                    <button onClick={() => window.dispatchEvent(new CustomEvent('switch-tab', { detail: 'billing' }))} className="px-6 py-2.5 rounded-[12px] bg-gradient-to-r from-emerald-500 to-cyan-500 text-white text-[13px] font-black shadow-md hover:scale-105 transition-all">升級方案 →</button>
                </div>
            ) : (
                <motion.button onClick={() => { setConfig((c: any) => ({ ...c, offerings: [...c.offerings, { ...EMPTY_OFFERING }] })); setTimeout(() => setExpanded(p => ({ ...p, [config.offerings.length]: true })), 50); }} className="w-full flex items-center justify-center gap-4 py-8 rounded-[24px] bg-gradient-to-r from-emerald-500 to-cyan-600 font-black text-white shadow-xl shadow-emerald-500/20 active:scale-95 text-[18px]">
