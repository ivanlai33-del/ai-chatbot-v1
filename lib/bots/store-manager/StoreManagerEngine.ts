...
// OS Foundation
import { trackEvent } from '@/lib/os/event-bus';
import { ContactService } from '@/lib/os/contact-service';
...
目前使用的 Line User ID: ${lineUserId}`
    ...
    if (is1199Plan) {
        ...
        if (hasReservationIntent) {
            ...
            // 🚀 異步處理：獲取聯絡人並拋出事件 (不阻塞回覆)
            (async () => {
                try {
                    // 1. 確保聯絡人主檔已建立 (一人一檔)
                    const contact = await ContactService.getOrCreateFromLine(bot.id, lineUserId);
                    ...
                    logger.info(`[OS Integration] Booking event emitted for contact ${contact.id}`);
                } catch (err) {
                    logger.error('[OS Integration] Failed to process modular booking', err);
                }
            })();
        }
    }
