...
async function run() {
    ...
    const { data: adminUser, error } = await supabase.from('direct_users').select('id').eq('line_user_id', 'Ud8b8dd79162387a80b2b5a4aba20f604').single();
    if (!adminUser) {
        console.log("Admin user not found in direct_users!", error);
        return;
    }
    ...
    console.log("Updated line_channel_configs:", configs?.length, configError || '');
